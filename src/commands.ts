...
export class Commands<C extends Context> {
    private _languages: Set<LanguageCode | 'default'> =
        new Set()
    private _scopes: Map<string, Array<Command<C>>> =
        new Map()
    ...
    private _addCommandToScope(
        ...
        const commands =
            this._scopes.get(JSON.stringify(scope)) ?? []
        this._scopes.set(
    ...
    public async setCommands({ api }: { api: Api }) {
        await Promise.all(
            this.toArgs().map((args) =>
                api.raw.setMyCommands(args)
            )
        )
    }
            ...
            .flatMap((command) => {
                ...
                for (const [
                    ...
                    elements.push({
                        ...
                        description:
                            command.getLocalizedDescription(
                                language
                            ),
                    })
                }
