  ...
  describe("hasCommand", () => {
    describe("default behavior", () => {
      ...
      it("should match regex commands", () => {
        ...
        assert(Command.hasCommand(/start_\d{3}/, { ...options, matchOnlyAtStart: false })(ctx));
    ...
    describe("matchOnlyAtStart", () => {
      it("should match a regular non-targeted command in the middle of the message", () => {
        ...
        m.entities = [{ type: "bot_command", offset: 7, length: 6 }];
        const ctx = new Context(update, api, me);
        assert(Command.hasCommand("start", { ...options, matchOnlyAtStart: false })(ctx));
      });
      ...
      it("should match a regular targeted command in the middle of the message", () => {
        ...
        assertFalse(Command.hasCommand("start", { ...options, matchOnlyAtStart: false })(ctx));
      });
    });
    ...
    describe("prefix", () => {
      it("should match a non-targeted command with a custom prefix", () => {
        ...
        m.entities = [];
        const ctx = new Context(update, api, me);
        assert(Command.hasCommand("start", { ...options, prefix: "!" })(ctx));

        m.text = "blabla !start";
        assertFalse(Command.hasCommand("start", { ...options, prefix: "!" })(ctx));
        assert(Command.hasCommand("start", { ...options, prefix: "!", matchOnlyAtStart: false })(ctx));
      });
      ...
      it("should ignore a non-existing command", () => {
        ...
        m.entities = [{ type: "bot_command", offset: 0, length: 6 }];
        const ctx = new Context(update, api, me);
        assertFalse(Command.hasCommand("other", { ...options, prefix: "!" })(ctx));
      });
    });
    ...
    describe("targetedCommands", () => {
      describe("ignored", () => {
        it("should match a non-targeted command", () => {
          ...
          m.entities = [{ type: "bot_command", offset: 0, length: 6 }];
          const ctx = new Context(update, api, me);
          assert(Command.hasCommand("start", { ...options, targetedCommands: "ignored" })(ctx));

          m.text = "blabla /start";
          m.entities = [{ type: "bot_command", offset: 7, length: 6 }];
          assertFalse(Command.hasCommand("start", { ...options, targetedCommands: "ignored" })(ctx));
          assert(
            Command.hasCommand("start", { ...options, targetedCommands: "ignored", matchOnlyAtStart: false })(ctx),
          );
        });
      ...
      describe("required", () => {
        it("should match a targeted command", () => {
          ...
          m.entities = [{ type: "bot_command", offset: 0, length: 10 }];
          const ctx = new Context(update, api, me);
          assert(Command.hasCommand("start", { ...options, targetedCommands: "required" })(ctx));

          m.text = "blabla /start@bot";
          m.entities = [{ type: "bot_command", offset: 7, length: 10 }];
          assert(
            Command.hasCommand("start", { ...options, targetedCommands: "required", matchOnlyAtStart: false })(ctx),
          );

          m.text = "/start@otherbot";
          m.entities = [{ type: "bot_command", offset: 0, length: 13 }];
          assertFalse(Command.hasCommand("start", { ...options, targetedCommands: "required" })(ctx));
        });
